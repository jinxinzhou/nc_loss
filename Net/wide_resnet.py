...
class Wide_ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, wfactor, num_classes=10, temp=1.0, etf=False, bias=True):
        ...
        self.fc = nn.Linear(64*block.expansion*wfactor, num_classes, bias=bias)
        print('bias:', bias)
        self.temp = temp
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if etf:
                    ...
                    m.weight = nn.Parameter(torch.mm(weight, torch.eye(num_classes, 64*block.expansion*wfactor)))
