...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1,width=1):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, width=1, num_classes=10, etf=False, bias=True, **kwargs):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, self.width))
...
def wide_resnet50_cifar(width=1, **kwargs):
    model = ResNet(Bottleneck, [3, 4, 6, 3], width=width, **kwargs)
    return model
